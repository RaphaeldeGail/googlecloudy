...
DOCUMENTATION = '''
---
module: gcp_resourcemanager_folder_iam
description:
- Sets the access control policy for a folder.
- The caller must have resourcemanager.folders.setIamPolicy permission on the identified folder.
short_description: Sets the access control policy for a GCP folder
...
options:
  folder_id:
    description:
    - The resource ID of the folder hosting the policy.
    - For example, 1234.
    required: true
    type: str
'''
...
EXAMPLES = '''
- name: Sets the access control policy for a GCP folder
...
def main():
    ...
    module = GcpModule(
        argument_spec=dict(
            ...
            folder_id=dict(required=True, type='str'),
        )
    )

    if not module.params['scopes']:
        module.params['scopes'] = ['https://www.googleapis.com/auth/cloudplatformfolders']
...
def self_link(module):
    return "{api}/folders/{folder_id}".format(api=API, **module.params)
