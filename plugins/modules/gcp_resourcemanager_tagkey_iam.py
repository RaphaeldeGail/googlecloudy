...
DOCUMENTATION = '''
---
module: gcp_resourcemanager_tagkey_iam
description:
- Sets the access control policy for a tagKey.
- The caller must have resourcemanager.tagKeys.setIamPolicy permission on the identified tagValue.
short_description: Sets the access control policy for a GCP tagKey
...
options:
  tagkey_id:
    description:
    - The resource ID of the tagKey hosting the policy.
    - For example, 1234.
    required: true
    type: str
'''
...
EXAMPLES = '''
- name: Sets the access control policy for a GCP tagKey
...
def main():
    ...
    module = GcpModule(
        argument_spec=dict(
            ...
            tagkey_id=dict(required=True, type='str'),
        )
    )
...
def self_link(module):
    return "{api}/tagKeys/{tagkey_id}".format(api=API, **module.params)
