...
DOCUMENTATION = '''
---
module: gcp_resourcemanager_organization_iam
description:
- Sets the access control policy for an organization.
- The current authenticated user must have the resourcemanager.organizations.setIamPolicy permission on the specified organization.
short_description: Sets the access control policy for a GCP organization
...
options:
  organization_id:
    description:
    - The resource ID of the organization hosting the policy.
    - For example, 1234.
    required: true
    type: str
'''
...
EXAMPLES = '''
- name: Sets the access control policy for a GCP organization
  raphaeldegail.googlecloudy.gcp_resourcemanager_organization_iam:
    ...
    - role: roles/my.other.role
      members:
      - user:user2@example.com
      - serviceAccount:service1@my_project_id.gserviceaccount.com
    polic_version: 1
...
def main():
    ...
    if not module.params['scopes']:
        module.params['scopes'] = ['https://www.googleapis.com/auth/cloudplatformorganizations']
...
def self_link(module):
    return "{api}/organizations/{organization_id}".format(api=API, **module.params)
